...
@Component({
  selector: 'FrmUpdateAlertEnvironnement',
  templateUrl: './FrmUpdateAlertEnvironnement.component.html',
  styleUrls: ['./FrmUpdateAlertEnvironnement.component.scss']
})
export class FrmUpdateAlertEnvironnementComponent implements OnInit {
  ...
  async ngOnInit() {
    this.route.params.subscribe(params => {
      this.idAlerte = params['id'];
    });
    await this.getSelectedAlerte();
  ...
  getSelectedAlerte() {
    let allParams = {
      ...
      primaryKeyName: "_id",
      primaryKeyValue: this.idAlerte
    };
    this.ystanceService.apiHandler(YstanceEndPoints.getItemByPrimaryKey, allParams).subscribe(this.onSuccessGetSelectedAlerte, YstanceHelper.onErrorResponse);
  }
  ...
  saveAlertEnvironment() {
    if (this.alertFirstWizardBuilder.valid && this.alertSecondWizardBuilder.valid) {
      ...
      let allParams = this.buildUpdateAlertEnvironment();
      this.ystanceService.apiHandler(YstanceEndPoints.updateItemByPrimaryKey, allParams).subscribe(this.onSuccessSaveAlert, YstanceHelper.onErrorResponse);
    }
    else {
  ...
  buildAlertEntity() {
    return {
      _id: this.idAlerte,
      id: this.selectedAlerte?.id || YstanceHelper.newGuid(), // Keep existing ID or generate new one
      date_alert: this.dateAlerte || this.selectedAlerte?.date_alert,
      category: this.selectedAlerte?.category,
      nature: this.nature || this.selectedAlerte?.nature,
      emetteur: this.emetteur || this.selectedAlerte?.emetteur,
      donor: this.alertFirstWizardBuilder.value.donor || this.selectedAlerte?.donor,
      datedecouverte: this.alertFirstWizardBuilder.value.datedecouverte || this.selectedAlerte?.datedecouverte,
      selectedPort: this.alertFirstWizardBuilder.value.selectedPort || this.selectedAlerte?.selectedPort,
      present: this.alertFirstWizardBuilder.value.present || this.selectedAlerte?.present,
      intervenant: this.alertFirstWizardBuilder.value.intervenant || this.selectedAlerte?.intervenant,
      nord: this.alertFirstWizardBuilder.value.nord || this.selectedAlerte?.nord,
      sud: this.alertFirstWizardBuilder.value.sud || this.selectedAlerte?.sud,
      type_intervention: this.alertFirstWizardBuilder.value.type_intervention || this.selectedAlerte?.type_intervention,
      sexe: this.alertSecondWizardBuilder.value.sexe || this.selectedAlerte?.sexe,
      observation: this.alertSecondWizardBuilder.value.observation_environments || this.selectedAlerte?.observation,
      listEspeces: this.listEspeces || this.selectedAlerte?.listEspeces || []
    };
  }
  ...
  onSuccessGetSelectedAlerte = (data: any) => {
    this.selectedAlerte = data;

    // Populate first wizard form
    this.alertFirstWizardBuilder.patchValue({
      donor: data.donor,
    ...
    // Populate second wizard form
    this.alertSecondWizardBuilder.patchValue({
      sexe: data.sexe,
      observation_environments: data.observation
    });

    // Populate species list if exists
    if (data.listEspeces && Array.isArray(data.listEspeces)) {
      this.listEspeces = data.listEspeces;
    }
  }
  ...
  onSuccessSaveAlert = (data: any) => {
    ...
    YstanceHelper.notify('Modifé avec succès', null, 'success', 1800, null);
  }
